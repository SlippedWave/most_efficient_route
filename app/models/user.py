...
class User(db.Model):
    ...
    USR_modified_by = db.Column(
        db.Integer, db.ForeignKey("users.USR_userId"), nullable=False
    )
    USR_last_modified = db.Column(
    ...
    modified_by = db.relationship(
        "User", remote_side=[USR_userId], backref="modified_users"
    )
    ...
    def __init__(
        ...
        USR_telephone=None,
        USR_address=None,
        USR_PER_permitId=None,
        USR_ST_statusId=None,
    ):
    ...
    def check_password(self, plain_password):
        return bcrypt.checkpw(
            plain_password.encode("utf-8"), self.USR_password.encode("utf-8")
        )
